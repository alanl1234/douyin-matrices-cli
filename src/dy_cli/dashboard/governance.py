"""Deterministic engagement governance (对齐 xiaohongshu-matrices-cli)。

AI / 批量输出在触达账号前，必经本引擎把关：
- PII 检测：手机号、微信号、地址、邮箱、身份证、QQ
- opt-out 检测：用户明确要求停止触达
- 高风险话术：违规承诺、刷量买粉、规避平台审核
- 内容相似度去重：防止同一内容跨账号重复发布 / 重复触达

所有判定都是确定性规则，AI 输出永远不能绕过这些检查。
"""
...
# 抖音语境下的拒绝 / 停止信号
OPT_OUT_PATTERNS = (
    re.compile(r"不要再联系|别联系|停止联系|不需要|别发了|退订|投诉|举报|取关"),
)

# 高风险 / 违规话术（平台红线）
HIGH_RISK_PATTERNS = (
    re.compile(r"包治|治愈|保证收益|稳赚|无风险|未成年|绕过审核|规避风控"),
    re.compile(r"刷量|刷赞|买粉|买赞|上热门保证|百分百热门|私域引流|导流微信"),
)
...
def evaluate_content(content: str, previous: Iterable[str] = (), threshold: float = 0.85) -> PolicyResult:
    """Evaluate content against governance rules.

    Args:
        content: candidate text (publish body / comment / DM).
        previous: recently published / sent texts for dedup.
        threshold: similarity above which content is treated as duplicate.

    Returns:
        PolicyResult with decision "allow" or "block".
    """
    reasons: list[str] = []
    ...
    if risk:
        reasons.append("检测到高风险效果承诺 / 规避平台安全机制的表述")
    if similarity > threshold:
        reasons.append(f"与近期内容相似度 {similarity:.2f} 超过阈值 {threshold:.2f}（疑似重复）")
...
def is_warm_lead(reason: str) -> bool:
    """Map an engagement reason to a warm-lead classification."""
